...
class Piece:
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        path = f"./pictures/{self.shortName}.png"
        ...
        screen.blit(
            img, (self.x * 64 + self.x_correction, self.y * 64 + self.y_correction)
        )

    def is_clicked(self, x: int, y: int) -> bool:
        if (
            x > self.x * 64 + self.x_correction
            and x < self.x * 64 + self.x_correction + self.image_size
        ):
            if (
                y > self.y * 64 + self.y_correction
                and y < self.y * 64 + self.y_correction + self.image_size
            ):
                return True
        return False
    ...
    def is_move_clicked(self, x: int, y: int) -> bool:
        for move in self.moves:
            if (
                x > move[0] * 64 + self.x_correction
                and x < move[0] * 64 + self.x_correction + self.image_size
            ):
                if (
                    y > move[1] * 64 + self.y_correction
                    and y < move[1] * 64 + self.y_correction + self.image_size
                ):
                    return True
        return False
    ...
    def show_moves(self, screen: pygame.Surface) -> None:
        for move in self.moves:
            pygame.draw.circle(
                ...
                (0, 0, 255),
                (
                    move[0] * 64 + self.x_correction + self.image_size // 2,
                    move[1] * 64 + self.y_correction + self.image_size // 2,
                ),
                5,
            )
